...
export class DataManagerAppStack extends cdk.Stack {
  constructor(scope: Construct, id: string, stageName: string, props?: cdk.StackProps) {
    ...
    const cluster = new ecs.Cluster(this, 'java-data-manager-cluster', {
      vpc,
    });

    // Build Docker image from the `code` directory
    const dockerImage = new ecrAssets.DockerImageAsset(this, 'java-data-manager-iocker-image', {
      directory: '../code', // Reference the 'code' directory for the Dockerfile
    });

    // Define Fargate service with an Application Load Balancer
    const fargateService = new ecsPatterns.ApplicationLoadBalancedFargateService(this, 'java-data-manager-fargate-service', {
      cluster,
      desiredCount: 1,
      cpu: 256,
      memoryLimitMiB: 512,
